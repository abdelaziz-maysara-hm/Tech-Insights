...
- Completed: Phases 0, 1, 1.5, 2, 3, 4 (4A–4E), 5A, and Phase 5B Batches 1–7.
- Next: continue Phase 5B priority REWORK content in another small verified batch -- 38 comparisons remain. **Recommended before further large content batches**: spot-check a sample of the current 47-item REMOVE list against the (now IAM-aware, but possibly still gapped in other domains) `professional`/`consumer` regexes -- Batch 7 found 5 genuinely valuable articles hiding in what looked like a clean REMOVE list, purely due to one missing vocabulary domain (IAM); other domains (e.g. cloud-native/Kubernetes terminology, OT/ICS security terminology) haven't been specifically audited for the same class of gap. Do not combine REWORK batches with consumer cleanup or removal decisions. Separately, Phase 6C (Alternatives pages) is scoped and ready to pick up as its own multi-batch effort whenever prioritized -- see ROADMAP.md.
