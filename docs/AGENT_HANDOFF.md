...
- Completed: Phases 0, 1, 1.5, 2, 3, 4 (4A–4E), 5A, and Phase 5B Batches 1–6.
- Next: continue Phase 5B priority REWORK content in another small verified batch -- 39 comparisons remain. All 8 comparisons checked so far needed only excerpt/verdict expansion with cited sources, not spec-table rewrites; expect a mix of both across the rest. Do not combine it with consumer cleanup or removal decisions. Separately, Phase 6C (Alternatives pages) is scoped and ready to pick up as its own multi-batch effort whenever prioritized -- see ROADMAP.md.
