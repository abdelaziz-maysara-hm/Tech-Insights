...
cats = ['cybersecurity', 'mobile', 'laptops', 'howto', 'ai', 'reviews', 'windows', 'comparisons']
extraTitles = []
for i in range(40):
    cat = cats[i % 8]
    extraTitles.append([f'موضوع إضافي وتقني عن {cat} رقم {i}', f'Additional tech topic about {cat} number {i}', cat])

allTitles = baseTitles + extraTitles

youtubeVideoIds = ['TX9qSaGXFyg', 'jfKfPfyJRdk', 'DC9cFkSnKz8', '8aGhZQkoFbQ', 'CquRUrFT6Zk', 'ysEN5RaKOlA', 'UquP3gK1AuQ', 'hvjmvJPCNpA', 'zQnBQ4tB3ZA', 'BhCAgweSKs0', 'kKSCDOSk4lk', 'eIho2S0ZahI', '_Ql9cA5jF0I', 'jfKfPfyJRdk', 'TX9qSaGXFyg']
authorNamesAr = ['أحمد محمود', 'سارة خالد', 'عمر طارق', 'نور أحمد', 'كريم حسن', 'ليلى سامي', 'خالد عبد الله']
authorNamesEn = ['Ahmed Mahmoud', 'Sarah Khaled', 'Omar Tarek', 'Nour Ahmed', 'Karim Hassan', 'Laila Sami', 'Khaled Abdallah']

output = "import { Article } from './mockData';\nimport { fullBody } from './mockData';\n\nexport const extendedArticles: Article[] = [\n"

for i, t in enumerate(allTitles):
    aid = 100 + i
    slug = f"article-{aid}-{t[1].lower().replace(' ', '-').replace(':', '').replace('\"', '')}"
    cat = t[2]
    arTitle = t[0].replace("'", "\\'")
    enTitle = t[1].replace("'", "\\'")
    
    hasVideo = (i % 10 < 4)
    videoId = youtubeVideoIds[i % len(youtubeVideoIds)] if hasVideo else None
    
    readTime = 3 + (i % 10)
    
    import datetime
    start = datetime.datetime(2023, 1, 1).timestamp()
    end = datetime.datetime(2024, 6, 30).timestamp()
    date_ts = start + (end - start) * (i / 200)
    date_str = datetime.datetime.fromtimestamp(date_ts).strftime('%Y-%m-%d')
    
    authorIdx = i % len(authorNamesAr)
    authorAr = authorNamesAr[authorIdx]
    authorEn = authorNamesEn[authorIdx]
    avatar = f"https://i.pravatar.cc/150?img={(i % 70) + 1}"
    
    isFeatured = "true" if i % 30 == 0 else "false"
    isTrending = "true" if i % 25 == 0 else "false"
    
    videoStr = f"\n    youtubeVideoId: '{videoId}'," if videoId else ""
    ...
    tags: ['Tech', '{cat.capitalize()}', '2024'],{videoStr}
...
with open("artifacts/techpulse-ar/src/data/articlesData.ts", "w", encoding="utf-8") as f:
    f.write(output)
